...
static constexpr long double total_comb() {
    ...
    for (int i = 1; i <= ERRORS; ++i) {
        ret += Combination(i, LENGTH - 1) * powl(31, i - 1);
    }
    return ret;
}
...
bool RecurseShortFaults(int pos, bool allzerobefore, Vector<ERRORS>& fault, const psol_type& psol_low, const psol_type& psol_high, const basis_type& basis, int part, uint64_t hash, LockedErrCount& err) {
    if (pos == ERRORS) {
    ...
    for (int x = 0; x < max; ++x) {
        fault[pos] = x ^ rrr;
        if (!RecurseShortFaults(pos + 1, allzerobefore && fault[pos] == 0, fault, psol_low, psol_high, basis, part, hash * 9672876866715837617ULL + fault[pos], err)) {
            return false;
        }
    }
    return true;
...
void run_thread(const psol_type* partials_low, const psol_type* partials_high, const basis_type* basis, int part, LockedErrCount* locs) {
    Vector<ERRORS> faults;
    (void)RecurseShortFaults(0, true, faults, *partials_low, *partials_high, *basis, part, 0, *locs);
}
...
bool testalot(const basis_type* basis, ErrCount* res) {
    ...
    std::array<int, ERRORS> pos;
    pos[0] = 0;
    RecursePositions(1, ERRORS, 1, LENGTH - 1, pos, partials_low, *basis);
    RecursePositions(0, ERRORS, 1, LENGTH, pos, partials_high, *basis);

    LockedErrCount ret;
#if THREADS > 1
    std::vector<std::thread> t;
    for (int part = 0; part < THREADS - 1; ++part) {
        t.emplace_back(&run_thread, &partials_low, &partials_high, basis, part, &ret);
    }
#endif
    run_thread(&partials_low, &partials_high, basis, THREADS - 1, &ret);
#if THREADS > 1
    for (int part = 0; part < THREADS - 1; ++part) {
        t[part].join();
    }
